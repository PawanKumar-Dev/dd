...
api.interceptors.request.use(
  (config) => {
    ...
    console.log(`🌐 [${timestamp}] ResellerClub DNS API Request:`, {
  ...
  (error) => {
    console.error("❌ ResellerClub DNS API Request Error:", error);
...
api.interceptors.response.use(
  (response: AxiosResponse) => {
    const timestamp = new Date().toISOString();
    console.log(`✅ [${timestamp}] ResellerClub DNS API Response:`, {
  ...
  (error: AxiosError) => {
    const timestamp = new Date().toISOString();
    console.error(`❌ [${timestamp}] ResellerClub DNS API Error:`, {
