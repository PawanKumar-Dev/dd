...
export class ResellerClubAPI {
  ...
  /**
   * Get domain pricing for specific TLDs (DEPRECATED - Use PricingService instead)
   */
  static async getDomainPricingLegacy(tlds: string[]): Promise<any> {
    const startTime = Date.now();
    console.log(
      `💰 [PRODUCTION] Starting domain pricing lookup for TLDs:`,
      tlds
    );

    try {
      const response = await api.get("/api/domains/pricing.json", {
        params: {
          tlds: tlds.join(","),
          action: "REGISTER",
        },
      ...
      const responseTime = Date.now() - startTime;
      console.log(
        `✅ [PRODUCTION] Domain pricing lookup completed in ${responseTime}ms:`,
        {
          tlds: tlds,
          responseStatus: response.status,
          dataKeys: Object.keys(response.data || {}),
      ...
      const responseTime = Date.now() - startTime;
      console.error(
        `❌ [PRODUCTION] Domain pricing lookup failed after ${responseTime}ms:`,
        {
          tlds: tlds,
          error: error instanceof Error ? error.message : "Unknown error",
          axiosError:
      ...
      if (error instanceof AxiosError) {
        if (error.response?.status === 401) {
          ...
          throw new Error("Invalid TLD list provided for pricing lookup.");
        } else if (error.response?.status >= 500) {
          throw new Error(
            "ResellerClub API server error. Please try again later."
          );
