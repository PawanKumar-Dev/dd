...
if (!RESELLERCLUB_API_URL || !RESELLERCLUB_API_ID || !RESELLERCLUB_API_KEY) {
  throw new Error("ResellerClub API configuration is missing. Please check your environment variables.");
}
...
export class PricingService {
  ...
  static async getDomainPricing(): Promise<any> {
    ...
    try {
      ...
      const customerTldCount = Object.keys(customerPricingResponse.data || {}).length;
      console.log(
        `✅ [PRICING] Customer pricing fetched in ${Date.now() - startTime}ms - ${customerTldCount} TLDs available`
      );
      ...
      const resellerTldCount = Object.keys(resellerPricingResponse.data || {}).length;
      console.log(
        `✅ [PRICING] Reseller pricing fetched in ${Date.now() - startTime}ms - ${resellerTldCount} TLDs available`
      );
    ...
    try {
      ...
      tlds.forEach((tld) => {
        ...
        if (foundTld) {
          ...
          console.log(`✅ [PRICING] Found ${cleanTld}: Customer ₹${customerPrice}, Reseller ₹${resellerPrice} (Margin: ${margin > 0 ? '+' : ''}${margin.toFixed(1)}%)`);
        }
      });
