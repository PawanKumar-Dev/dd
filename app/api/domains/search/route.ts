...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const livePricingCount = results.filter(r => r.pricingSource === 'live').length;
    
    console.log(
      `✅ [API-${requestId}] Domain search completed in ${
        Date.now() - startTime
      }ms - ${results.length} domains found, ${availableCount} available, ${livePricingCount} with live pricing`
    );
